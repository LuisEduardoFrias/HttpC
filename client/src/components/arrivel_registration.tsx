...
export default function ArrivelRegistration({ token }: { token: string }) {
  ...
  useEffect(() => {
    setFetch({
      url: `arrival_registration/${token}`,
    });
  }, [])
  ...
  return (
    <div className='arrivel-reg-container'>
      ...
      />
      <TableX data={data.data} headers={["Id", "User id", "Date Time", "Input/output"]}/>
    </div>
  );
}
